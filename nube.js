  ...
  card.innerHTML = `
    ...
    <div class="card-body">
      ...
      <div class="card-actions">
        ...
        <button class="btn btn-primary" data-vote-id="${id}" data-vote-name="${escapeHtml(data.name)}" data-vote-image="${data.imageUrl||''}">Votar</button>
      </div>
    </div>
  `;
...
window.addEventListener('DOMContentLoaded', ()=>{
  try {
    ...
    (async function showAlumnoName(){
      ...
      } catch(e) { console.error('[nube] showAlumnoName error', e); }
    })();
    ...
    // delegación: cuando se haga click en un botón de votar, abrir modal
    document.body.addEventListener('click', (ev)=>{
      ...
      // almacenar en dataset del modal para referencia
      if (voteModal) voteModal.dataset.voteId = gid;
      if (voteModal) voteModal.style.display = 'flex';
    });

    if (cancelVoteBtn) cancelVoteBtn.addEventListener('click', ()=>{ if (voteModal) voteModal.style.display='none'; });
  ...
  try {
    ...
    if (loginBtn) {
      loginBtn.addEventListener("click", async () => {
        ...
        const adminsRef = ref(db, "admins/" + user);
        ...
        try {
          ...
          if (snapshot.exists()) {
            ...
            if (data.password === pass) {
              try { sessionStorage.setItem('adminAuth', user); } catch(e){}
              try { localStorage.setItem('adminAuth', user); } catch(e){}
              try { sessionStorage.removeItem('panelBlocked'); } catch(e){}
              window.location.href = "panel.html";
            } else {
              if (loginMsg) { loginMsg.textContent = "Contraseña incorrecta."; loginMsg.style.display = "block"; }
            }
          } else {
            if (loginMsg) { loginMsg.textContent = "Usuario no encontrado."; loginMsg.style.display = "block"; }
          }
        } catch (error) {
          const errText = (error && (error.message || error.toString())) || 'Error desconocido';
          if (loginMsg) { loginMsg.textContent = `Error de conexión con Firebase: ${errText}`; loginMsg.style.display = 'block'; }
